...
- **Week 3 — two findings + three predictions, all with prompts written in place.** ⏳ *Newest, and the most perishable — he watched these runs.* The leak (D11), the straw-man baseline, benchmark row 2, and now the T4 row. **Prediction #2 is reconciled and was correct**; one follow-up is left open beneath it: *if batch 8 fits, should you use it?* — at batch 8 an hour is many more epochs over the same 276 images, and the question is what stops being true when a model sees the same pictures twenty times instead of three. **Prediction #4 is owed before W4 runs** — the ordering of QLoRA / LoRA / DoRA under a matched wall-clock budget, and whether fp16 LoRA's larger footprint helps or hurts when the budget is time.
- **Milestone F — prediction reconciled, three questions open.** ⏳ He predicted both outcomes **correctly** (the scale falls; a gradient goes non-finite before the loss) — but both answers were a bare direction with no mechanism, so neither could be partly right or partly wrong. The reconcile is written; the three questions are (1) why can no single-step check separate the baseline's 6 harmless skips from sabotage's 5 fatal ones, (2) what would have happened on Kaggle had the tripwire only checked the loss, (3) why did steps stop landing after step 2, and why is a model that has stopped changing more dangerous than one visibly exploding. *(The Week 3 batch-size prediction once listed here was written 2026-08-14 and reconciled 2026-08-15 — **correct on both counts**. See the Week 3 section of the log.)*
- ⚠️ **The standing pattern, now with three data points and a cost attached:** predictions give a direction and no mechanism. Milestone F's two were both **right**; prediction #2 ("batch 8, VRAM is the constraint") was **also right** — and still could not be checked against anything except whether 8 appeared. What the probe supplied and the prediction did not: **0.752 GiB per example against a 2.69 GiB fixed cost**, which is *why* the ceiling is single-digit and which transfers to LoRA and DoRA in W4. A correct number that carries no mechanism does not generalize to the next run; that is the whole cost of the pattern, and W4's prediction #4 is the place to break it.
...
| **`notebooks/kaggle_qlora_train_t4_run.ipynb`** | **W3 RUN RECORD.** The executed notebook with all console output captured — the *only* surviving artifact of the T4 run (§12). Not a launcher; do not run it |
...
- **2026-08-15 — Week 3 ✅ COMPLETE on a T4. Benchmark row 2 measured: 58.9% strict vs the 31.9% per-type prior = +27.0 pp**, McNemar p = 6.4e-24 (b=65, c=243), 6,924 optimizer steps in a 60 min budget, peak 3.60 GiB. Format compliance 100%, binary 77.2%, open-ended 43.3%. Full record in §12; the executed run record is `notebooks/kaggle_qlora_train_t4_run.ipynb`.
  - 🚨 **A wall-clock budget makes ACCURACY device-bound, not just the timing columns.** Identical code, data and effective batch: the 3060 fitted **2,428** steps (1 epoch) into the hour and scored 47.2%; the T4 fitted **6,924** (3 epochs) and scored **58.9%**. Device speed converts into steps and steps into accuracy, so **+11.7 pp of this row is hardware, not method.** `eval-protocol.md` §7 already said device-bound quantities compare only within a device — this shows accuracy is one of them. **Every W4 row must be a T4 row.**
  - **`requirements-kaggle.txt` ran for the first time, and failed on the axis nobody guarded.** torch was exactly right (2.10.0+cu128, no drift under the pinned environment). The break was **torchao 0.10.0**, already in the base image, unmentioned by the file, and never installed on Windows: peft's `is_torchao_available()` **raises** instead of returning `False`, and LoRA injection calls it per module. **peft 0.20.0 carries the same raise**, so matching the local pin would not have helped. **A requirements file only defends against the versions it names.** Third instance of right-concern/wrong-mechanism, after §6's uint8/int32-offset and §10's ViT gradients-not-activations.
  - **Prediction #2 was correct on both counts** — batch 8 fits at 8.70 GiB, 16 OOMs. The curve gave what the number could not: **0.752 GiB per example vs 2.69 GiB fixed**, three quarters of a gigabyte of retained activations for a ~101-token sequence. A pre-run caution is recorded as **wrong**: `seq_len` was expected to climb with batch and went 101 → 104, so padding costs almost nothing and the linear fit was sounder than warned.
  - 🚨 **The results JSONs and the adapter were LOST.** The interactive session was torn down before Save Version ran, so `/kaggle/working` was empty at save time and `kaggle kernels files` returns nothing. Every printed metric survives in the committed notebook; the 659 per-example records do not, so **W4 cannot run a paired test against this row and must re-run QLoRA on the T4.** **The fix is "Save Version → Save & Run All (Commit)"**, which executes in a batch session and persists `/kaggle/working` automatically — interactive Run All persists nothing unless a version is saved *while the session still holds the files*, which the notebook's own teardown warning does not say.
  - **~35 min of GPU quota was spent rebuilding data with the GPU at 0%.** `outputs/fw_pixels.zip` (157 MB, gitignored) is now built and ready to attach as a Kaggle Dataset — §6's plan from Milestone D, still undone, and W4 pays that cost three more times without it.
  - **Two Kaggle accounts exist and only one was ever written down.** The project account is `aadipathak2323@gmail.com` = **`aadipathak12`**; the browser's default Google session is a different account, `aadipathak`, and `kaggle auth login` silently authenticated as it — producing `Permission 'kernels.get' was denied` on a notebook that plainly existed. Credentials cache at `~/.kaggle/credentials.json`; delete it to switch. The `kaggle` CLI is now installed in the venv and authenticated as `aadipathak12`.
